...
const registerSocketHandlers = (io) => {
  const namespaces = [io.of("/"), io.of("/syncpad")];

  namespaces.forEach((nsp) => {
    nsp.on("connection", (socket) => {
      console.log(`Device connected to namespace ${nsp.name}: ${socket.id}`);
      ...
      const checkAndTransferLock = async (sheetId) => {
        ...
        const socketsInRoom = await nsp.in(`sheet_${sheetId}`).fetchSockets();
        ...
        if (eligibleSockets.length > 0) {
          ...
          nsp.to(`sheet_${sheetId}`).emit("sheet_lock_status", {
            isLocked: true,
            lockedBy: nextSocket.username,
            lockedBySocketId: nextSocket.id,
          });
          console.log(
            `Lock for sheet ${sheetId} transferred to ${nextSocket.username} (${nextSocket.id}) in namespace ${nsp.name}`,
          );
        } else {
          sheetLocks.delete(sheetId);
          console.log(`Lock for sheet ${sheetId} released (no active viewers) in namespace ${nsp.name}`);
        }
      };
      ...
      socket.on("client_viewing_sheet", async ({ sheetId, username }) => {
        try {
          ...
          console.log(
            `Device ${socket.id} (${username}) is now viewing sheet: ${sheetId} in namespace ${nsp.name}`,
          );
          ...
          if (sheet.status === "live") {
            ...
            if (lock) {
              const activeSocket = nsp.sockets.get(lock.socketId);
      ...
      socket.on("client_take_control_sheet", async ({ sheetId, username }) => {
        try {
          ...
          if (sheet.status === "live") {
            ...
            console.log(
              `User ${username} (${socket.id}) took control of sheet ${sheetId} in namespace ${nsp.name}`,
            );
          }
        } catch (err) {
      ...
      socket.on("client_sheets_list_modified", () => {
        nsp.emit("sheets_list_updated");
      });

      socket.on("disconnect", async () => {
        console.log(`Device disconnected from namespace ${nsp.name}: ${socket.id}`);
